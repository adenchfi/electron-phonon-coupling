...
def bndplot(datafile,fermi,symmetryfile,subplot,label):
  ...
  Fermi = float(fermi)
  axis = [min(x),max(x),Fermi - 8, Fermi + 6]
...
bndplot('bands.out.gnu',0.0,'bands.out',BND,'test') #This is my bndplot with *.gnu coming from the output of bands.x, 2.1082 is the fermi level, bandx.out is the symmetries file, and 'test' is the title
ylim = BND.get_ylim()
dosplot('.',DOS,0.0) #This is my DOS plot with 0.0 being the fermi level
