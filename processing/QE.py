  ...
  def File_Process(self,filestring):
    ...
    for i in f:
      ...
      if "cell_parameters" in i:
        ...
        self.Normalize()
      if "ATOMIC_POSITIONS" in i.upper(): # might be the problem
        self.atomindex.reset()
        if "angstrom" in i:
          for j in range(0,self.natoms):
            line = next(f).split()
      ...
      if "begin final coordinates" in i.lower(): # might be the problem
        self.atomindex.reset()
        line = next(f).split()
        line = next(f).split() # should get me to the "ATOMIC POSITIONS" line
        print(line)
        if "angstrom" in i:
          for j in range(0,self.natoms):
            line = next(f).split()
        ...
        if "alat" in i:
          for j in range(0,self.natoms):
            line = next(f).split()
            self.atoms[self.atomindex.key(line[0])] = np.array([self.alat*float(line[1]),self.alat*float(line[2]),self.alat*float(line[3])])
        if ("ATOMIC_POSITIONS" in i.upper() or "(crystal)" in line[1]):
          ...
          for j in range(0,self.natoms):
            ...
            #print(self.atomindex.key(line[0]))
